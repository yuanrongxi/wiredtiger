...
/*从上层的application中读取columns到对应的cursor中*/
int __wt_schema_project_in(WT_SESSION_IMPL *session, WT_CURSOR **cp, const char *proj_arg, va_list ap)
{
	WT_CURSOR *c;
	WT_DECL_ITEM(buf);
...
/*从对应的cursors中读取columns的信息，并返回给上层的application*/
int __wt_schema_project_out(WT_SESSION_IMPL *session, WT_CURSOR **cp, const char *proj_arg, va_list ap)
{
	WT_CURSOR *c;
	WT_DECL_PACK(pack);
	WT_DECL_PACK_VALUE(pv);
...
/*从一个raw buffer中读取所有的column的信息到对应的cursor中*/
int __wt_schema_project_slice(WT_SESSION_IMPL *session, WT_CURSOR **cp, const char *proj_arg, int key_only, const char *vformat, WT_ITEM *value)
{
	WT_CURSOR *c;
	WT_DECL_ITEM(buf);
	WT_DECL_PACK(pack);
		...
		for (arg = (arg == 0) ? 1 : arg; arg > 0; arg--) {
			...
			case WT_PROJ_SKIP:
				...
				if (p == end) {
					...
					WT_RET(__unpack_read(session, &pv, (const uint8_t **)&p,
					    (size_t)(end - p)));
				break;
			...
			case WT_PROJ_REUSE:
				...
				WT_ASSERT(session, tolower(pv.type) == tolower(vpv.type));
				pv.u = vpv.u;
...
/*从cursor中读取所有column中的k/v信息，并将这些信息格式化后填入一个raw buffer中*/
int __wt_schema_project_merge(WT_SESSION_IMPL *session, WT_CURSOR **cp, const char *proj_arg, const char *vformat, WT_ITEM *value)
{
	WT_CURSOR *c;
	WT_ITEM *buf;
	WT_DECL_PACK(pack);
