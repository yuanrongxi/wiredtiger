...
int __wt_schema_colgroup_name(WT_SESSION_IMPL *session, WT_TABLE *table, const char *cgname, size_t len, WT_ITEM *buf)
{
...
int __wt_schema_open_colgroups(WT_SESSION_IMPL* session, WT_TABLE* table)
{
	...
	for(i = 0; i < WT_COLGROUPS(table); i++){
		...
		WT_ERR(__wt_strndup(session, buf->data, buf->size, &colgroup->source));
	...
	if(!table->is_simple){
		...
		WT_ERR(__wt_buf_init(session, buf, 0));
		WT_ERR(__wt_struct_plan(session, table, table->colconf.str, table->colconf.len, 1, buf));
...
static int __open_index(WT_SESSION_IMPL *session, WT_TABLE *table, WT_INDEX *idx)
{
	...
	if (cval.len != 0) {
		...
		WT_ERR(__wt_collator_config(session, idx->name, &cval, &metadata, &idx->collator, &idx->collator_owned));
	}

	WT_ERR(__wt_extractor_config(session, idx->config, &idx->extractor, &idx->extractor_owned));
	 ...
	 */
	WT_ERR(__wt_buf_init(session, buf, 0));
	WT_ERR(__wt_config_getones(session, idx->config, "columns", &idx->colconf)); /*确定索引关联的column*/
	...
	for (i = 0; i < table->nkey_columns && (ret = __wt_config_next(&colconf, &ckey, &cval)) == 0; i++) {
		/*
	...
	WT_ERR(__wt_struct_plan(session, table, table->colconf.str, table->colconf.len, 1, plan));
...
/*打开表的所有索引对象*/
int __wt_schema_open_index(WT_SESSION_IMPL *session, WT_TABLE *table, const char *idxname, size_t len, WT_INDEX **indexp)
{
	WT_CURSOR *cursor;
	WT_DECL_ITEM(tmp);
	WT_DECL_RET;
	...
	for (i = 0; ret == 0; i++, ret = cursor->next(cursor)) {
		 ...
		 */
		WT_ERR(__wt_realloc_def(session, &table->idx_alloc, WT_MAX(i, table->nindices) + 1, &table->indices));
		...
		while (table->indices[i] != NULL && (cmp = strcmp(uri, table->indices[i]->name)) > 0) {
			...
			memmove(&table->indices[i], &table->indices[i + 1], (table->nindices - i) * sizeof(WT_INDEX *));
			table->indices[--table->nindices] = NULL;
		}
		if (cmp < 0) {
			/* Make room for a new index. */
			memmove(&table->indices[i + 1], &table->indices[i], (table->nindices - i) * sizeof(WT_INDEX *));
...
/*在内存中创建一个表的schema对象*/
int __wt_schema_open_table(WT_SESSION_IMPL *session, const char *name, size_t namelen, int ok_incomplete, WT_TABLE **tablep)
{
	WT_CONFIG cparser;
	WT_CONFIG_ITEM ckey, cval;
	WT_CURSOR *cursor;
	...
	WT_ERR(__wt_config_getones(session, table->config, "columns", &table->colconf));
	...
	if (!table->is_simple)
		WT_ERR(__wt_schema_colcheck(session, table->key_format, table->value_format, &table->colconf, &table->nkey_columns, NULL));

	WT_ERR(__wt_config_getones(session, table->config, "colgroups", &table->cgconf));
	...
	if (!ok_incomplete && !table->cg_complete)
		WT_ERR_MSG(session, EINVAL, "'%s' cannot be used until all column groups are created", table->name);
...
/*通过uri找到对应的column group对象*/
int __wt_schema_get_colgroup(WT_SESSION_IMPL *session, const char *uri, int quiet, WT_TABLE **tablep, WT_COLGROUP **colgroupp)
{
	WT_COLGROUP *colgroup;
	WT_TABLE *table;
	...
	*colgroupp = NULL;
	...
	WT_RET(__wt_schema_get_table(session, tablename, WT_PTRDIFF(tend, tablename), 0, &table));
...
/*通过索引的uri找到索引的对象,如果没在内存中找到对应的索引对象，就创建一个新的索引对象关联对应的表*/
int __wt_schema_get_index(WT_SESSION_IMPL *session, const char *uri, int quiet, WT_TABLE **tablep, WT_INDEX **indexp)
{
	WT_DECL_RET;
	WT_INDEX *idx;
	WT_TABLE *table;
