...
/*通过config_name获得配置信息中一个长度，并判断这个长度是否是direct I/O长度的整数倍，这个是和磁盘的扇区写入有关系*/
int __wt_direct_io_size_check(WT_SESSION_IMPL *session, const char **cfg, const char *config_name, uint32_t *allocsizep)
{
	WT_CONFIG_ITEM cval;
	...
	*allocsizep = 0;
	 ...
	 */
	if(FLD_ISSET(conn->direct_io, WT_FILE_TYPE_CHECKPOINT | WT_FILE_TYPE_DATA)){
		align = (int64_t)conn->buffer_alignment;
		if (align != 0 && (cval.val < align || cval.val % align != 0))
			WT_RET_MSG(session, EINVAL, 
			"when direct I/O is configured, the %s size must be at least as large as the buffer alignment "
			"as well as a multiple of the buffer alignment",
			config_name);
	}
...
static int __create_file(WT_SESSION_IMPL *session, const char *uri, int exclusive, const char *config)
{
	WT_DECL_ITEM(val);
	WT_DECL_RET;
	uint32_t allocsize;
	int is_metadata;
	const char *filename, **p, *filecfg[] = { WT_CONFIG_BASE(session, file_meta), config, NULL, NULL };
	char *fileconf;
	...
	filename = uri;
	if(!WT_PREFIX_SKIP(filename, "file:")) /*跳过前缀*/
		WT_RET_MSG(session, EINVAL, "Expected a 'file:' URI: %s", uri);
	
	/*如果不是meta文件的schema操作，那么进行meta元数据查找*/
	...
	/*检查allocation_size是否能支持direct I/O对齐*/
	WT_RET(__wt_direct_io_size_check(session, filecfg, "allocation_size", &allocsize));
...
/*获取column group 对应data source的uri*/
int __wt_schema_colgroup_source(WT_SESSION_IMPL *session, WT_TABLE *table, const char *cgname, const char *config, WT_ITEM *buf)
{
	WT_CONFIG_ITEM cval;
	WT_DECL_RET;
	size_t len;
	...
	ret = __wt_config_getones(session, config, "type", &cval);
	if(ret == 0 && !WT_STRING_MATCH("file", cval.str, cval.len)){
		prefix = cval.str;
		len = cval.len;
		suffix = "";
	}
	...
	if(cgname == NULL)
		...
		WT_RET(__wt_buf_fmt(session, buf, "%.*s:%s_%s%s", (int)len, prefix, tablename, cgname, suffix));

	return 0;
}

/*创建一个column group对象*/
static int __create_colgroup(WT_SESSION_IMPL* session, const char* name, int exclusive, const char* config)
{
	WT_CONFIG_ITEM cval;
	WT_DECL_RET;
	...
	/*通过表名获得对应的表对象*/
	if ((ret =__wt_schema_get_table(session, tablename, tlen, 1, &table)) != 0)
	...
	if (cgname != NULL && (ret =__wt_config_subgets(session, &table->cgconf, cgname, &cval)) != 0)
		WT_ERR_MSG(session, EINVAL, "Column group '%s' not found in table '%.*s'", cgname, (int)tlen, tablename);
	...
	if (cgname == NULL)
		...
		WT_ERR(__wt_struct_reformat(session, table, cval.str, cval.len, NULL, 1, &fmt));
	}
...
/*获取索引对应datasource的uri文件地址*/
int __wt_schema_index_source(WT_SESSION_IMPL* session, WT_TABLE* table, const char *idxname, const char *config, WT_ITEM *buf)
{
	WT_CONFIG_ITEM cval;
	WT_DECL_RET;
	size_t len;
	...
	tablename = table->name + strlen("table:");
	if((ret == __wt_config_getones(session, config, "type", &cval)) == 0 && !WT_STRING_MATCH("file", cval.str, cval.len)){
		prefix = cval.str;
		len = cval.len;
		suffix = "_idx";
	}
	...
	WT_RET(__wt_buf_fmt(session, buf, "%.*s:%s_%s%s", (int)len, prefix, tablename, idxname, suffix));

	return 0;
}

/*创建索引对象*/
static int __create_index(WT_SESSION_IMPL* session, const char* name, int exclusive, const char* config)
{
	...
	if (idxname == NULL)
		WT_RET_MSG(session, EINVAL, "Invalid index name, should be <table name>:<index name>: %s", name);
	/*获得表对象*/
	tlen = (size_t)(idxname++ - tablename);
	if ((ret = __wt_schema_get_table(session, tablename, tlen, 1, &table)) != 0)
		WT_RET_MSG(session, ret, "Can't create an index for a non-existent table: %.*s", (int)tlen, tablename);

	if (table->is_simple)
		WT_RET_MSG(session, EINVAL, "%s requires a table with named columns", name);

	/*获取对应索引的datasource的URI*/
	...
	for (i = 0; i < table->nkey_columns && (ret = __wt_config_next(&pkcols, &ckey, &cval)) == 0; i++) {
		/*
		...
		if (__wt_config_subgetraw(session, &icols, &ckey, &cval) == 0) {
			if (have_extractor)
				WT_ERR_MSG(session, EINVAL, "an index with a custom extractor may not include primary key columns");
			continue;
		}
	 ...
	 */
	WT_ERR(__wt_struct_reformat(session, table, icols.str, icols.len, (const char *)extra_cols.data, 0, &fmt));
	...
	if (cval.len == 1 && cval.str[0] == 'r')
		WT_ERR_MSG(session, EINVAL, "column-store index may not use the record number as its index key");
...
/*创建表对象*/
static int __create_table(WT_SESSION_IMPL* session, const char* name, int exclusive, const char* config)
{
	WT_CONFIG conf;
	...
	if ((ret = __wt_schema_get_table(session, tablename, strlen(tablename), 0, &table)) == 0) {
		__wt_schema_release_table(session, table);
	...
	/*尝试性的打开刚刚创建的table对象*/
	WT_ERR(__wt_schema_get_table(session, tablename, strlen(tablename), 1, &table));
...
/*创建一个自定义的data source*/
static int __create_data_source(WT_SESSION_IMPL* session, const char* uri, const char* config, WT_DATA_SOURCE* dsrc)
{
	WT_CONFIG_ITEM cval;
	 ...
	 */
	if (__wt_config_getones_none(session, config, "collator", &cval) != WT_NOTFOUND && cval.len != 0)
...
int __wt_schema_create(WT_SESSION_IMPL *session, const char *uri, const char *config)
{
	...
	if (WT_PREFIX_MATCH(uri, "colgroup:"))
		...
		ret = dsrc->create == NULL ?__wt_object_unsupported(session, uri) : __create_data_source(session, uri, config, dsrc);
	else
