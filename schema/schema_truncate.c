...
/*WT_SESSION::truncate文件的实现,清空文件操作*/
static __truncate_file(WT_SESSION_IMPL* session, const char* name)
{
	WT_DECL_RET;
	const char *filename;
	uint32_t allocsize;
	...
	WT_RET(__wt_session_get_btree(session, name, NULL, NULL, WT_DHANDLE_EXCLUSIVE));
	...
	WT_WITH_DHANDLE_LOCK(session, ret = __wt_conn_dhandle_close_all(session, name, 0));
	WT_RET(ret);
...
/*WT_SESSION::truncate表的实现，清空表操作*/
static int __truncate_table(WT_SESSION_IMPL* session, const char* name, const char* cfg[])
{
	WT_DECL_RET;
	WT_TABLE *table;
	u_int i;
...
int __wt_schema_truncate(WT_SESSION_IMPL *session, const char *uri, const char *cfg[])
{
	...
	if (WT_PREFIX_MATCH(uri, "file:"))
		...
		ret = dsrc->truncate == NULL ? __truncate_dsrc(session, uri) : dsrc->truncate(dsrc, &session->iface, uri, (WT_CONFIG_ARG *)cfg);
	else
