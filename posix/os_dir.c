...
int __wt_dirlist(WT_SESSION_IMPL *session, const char *dir, const char *prefix,
    uint32_t flags, char ***dirlist, u_int *countp)
{
	...
	*countp = 0;
	...
	WT_ERR(__wt_verbose(session, WT_VERB_FILEOPS,
	    "wt_dirlist of %s %s prefix %s",path, LF_ISSET(WT_DIRLIST_INCLUDE) ? "include" : "exclude", prefix == NULL ? "all" : prefix));
	...
	for (dirsz = 0, count = 0; (dp = readdir(dirp)) != NULL;) {
		...
		if (prefix != NULL &&
		    ((LF_ISSET(WT_DIRLIST_INCLUDE) && WT_PREFIX_MATCH(dp->d_name, prefix)) ||
		    (LF_ISSET(WT_DIRLIST_EXCLUDE) && !WT_PREFIX_MATCH(dp->d_name, prefix))))
			match = 1;
		...
		if (prefix == NULL || match) {
			...
			count++;
			if (count > dirsz) { /*还有文件，扩大entries的空间*/
				dirsz += WT_DIR_ENTRY;
				WT_ERR(__wt_realloc_def(session, &dirallocsz, dirsz, &entries));
			}

			WT_ERR(__wt_strdup(session, dp->d_name, &entries[count-1]));
		}
	}
	if (count > 0)
		*dirlist = entries;
