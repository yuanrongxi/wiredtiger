 ...
 */
static int __config_merge_scan(WT_SESSION_IMPL *session, const char *key, const char *value, WT_CONFIG_MERGE *cp)
{
	WT_CONFIG cparser;
	WT_CONFIG_ITEM k, v;
	WT_DECL_ITEM(kb);
	...
	while ((ret = __wt_config_next(&cparser, &k, &v)) == 0) {
		...
		for (len = 0; len < k.len; ++len)
			if (k.str[len] == SEPC)
				WT_ERR_MSG(session, EINVAL, "key %.*s contains a '%c' separator character", (int)k.len, (char *)k.str, SEPC);
		...
		if (v.type == WT_CONFIG_ITEM_STRUCT && strchr((const char* )vb->data, '=') != NULL) {
			WT_ERR(__config_merge_scan(session, (const char*)kb->data, (const char*)vb->data, cp));
			continue;
		}

		/* Insert the value into the array. */
		WT_ERR(__wt_realloc_def(session, &cp->entries_allocated, cp->entries_next + 1, &cp->entries));
 ...
 */
static int __config_merge_format_next(WT_SESSION_IMPL *session, const char *prefix, size_t plen, size_t *enp, WT_CONFIG_MERGE *cp, WT_ITEM *build)
{
 ...
 */
static int __config_merge_format(WT_SESSION_IMPL *session, WT_CONFIG_MERGE *cp, const char **config_ret)
{
	WT_DECL_ITEM(build);
	WT_DECL_RET;
	size_t entries;
 ...
 */
int __wt_config_merge(WT_SESSION_IMPL *session, const char **cfg, const char **config_ret)
{
	 ...
	 * generation.
	 */
	qsort(merge.entries, merge.entries_next, sizeof(WT_CONFIG_MERGE_ENTRY), __config_merge_cmp);
