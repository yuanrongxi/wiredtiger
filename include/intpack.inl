/**************************************************************************
 * 整型数pack-encoding算法,这个算法的宗旨是将pack时产生的0进行压缩，防止过
 * 多的0占用空间
 *
...
/*因为大部分数值都在[-2^13,  2^13]之间，这样做是为了更省空间*/
...
#define	WT_SIZE_CHECK(l, maxl)		WT_RET_TEST((maxl) != 0 && (size_t)(l) > (maxl), ENOMEM)

/*确定数字X前面单个字节0值的连续个数，i表示个数*/
#if defined(__GNUC__)
