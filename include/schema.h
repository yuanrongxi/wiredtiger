...
struct __wt_table {
	...
	WT_COLGROUP **cgroups;					/*列式存储的column group*/
...
#define	WT_WITH_DHANDLE_LOCK(session, op)				\
	WT_WITH_LOCK(session, &S2C(session)->dhandle_lock, WT_SESSION_HANDLE_LIST_LOCKED, op)
...
#define	WT_WITH_SCHEMA_LOCK(session, op) do {					\
	...
	WT_WITH_LOCK(session,&S2C(session)->schema_lock, WT_SESSION_SCHEMA_LOCKED, op);	\
} while (0)
...
#define	WT_WITHOUT_LOCKS(session, op) do {			\
	...
	int __handle_locked = F_ISSET(session, WT_SESSION_HANDLE_LIST_LOCKED);\
	int __schema_locked = F_ISSET(session, WT_SESSION_SCHEMA_LOCKED);	\
	if (__handle_locked) {					\
...
#define	WT_WITH_TABLE_LOCK(session, op) do {				\
	WT_ASSERT(session, F_ISSET(session, WT_SESSION_TABLE_LOCKED) ||	!F_ISSET(session, WT_SESSION_HANDLE_LIST_LOCKED));		\
	WT_WITH_LOCK(session, &S2C(session)->table_lock, WT_SESSION_TABLE_LOCKED, op);	\
} while (0)
