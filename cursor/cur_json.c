...
static size_t		__json_unpack_put(WT_SESSION_IMPL *, void *, u_char *, size_t, WT_CONFIG_ITEM *);
static inline int	__json_struct_size(WT_SESSION_IMPL *, const void *, size_t, const char *, WT_CONFIG_ITEM *, int, size_t *);
static inline int	__json_struct_unpackv(WT_SESSION_IMPL *, const void *, size_t, const char *, WT_CONFIG_ITEM *, u_char *, size_t, int, va_list);
...
static int			__json_pack_size(WT_SESSION_IMPL *, const char *, WT_CONFIG_ITEM *, int, const char *, size_t *);
 ...
 */
static size_t __json_unpack_put(WT_SESSION_IMPL *session, void *voidpv, u_char *buf, size_t bufsz, WT_CONFIG_ITEM *name)
{
	WT_PACK_VALUE *pv;
	const char *p, *end;
	size_t s, n;
 ...
 *	Calculate the size of a packed byte string as formatted for JSON.
 */
static inline int __json_struct_size(WT_SESSION_IMPL *session, const void *buffer,
    size_t size, const char *fmt, WT_CONFIG_ITEM *names, int iskey, size_t *presult)
{
	WT_CONFIG_ITEM name;
...
static inline int __json_struct_unpackv(WT_SESSION_IMPL *session,
    const void *buffer, size_t size, const char *fmt, WT_CONFIG_ITEM *names, u_char *jbuf, size_t jbufsize, int iskey, va_list ap)
{
	WT_CONFIG_ITEM name;
	WT_DECL_PACK_VALUE(pv);
	WT_DECL_RET;
 ...
 */
int __wt_json_alloc_unpack(WT_SESSION_IMPL *session, const void *buffer,
    size_t size, const char *fmt, WT_CURSOR_JSON *json, int iskey, va_list ap)
{
 ...
 */
size_t __wt_json_unpack_char(char ch, u_char *buf, size_t bufsz, int force_unicode)
{
	char abbrev;
	u_char h;
 ...
 *	of column names.
 */
int __wt_json_column_init(WT_CURSOR *cursor, const char *keyformat,
 ...
 *		'F'	:  false
 */
int __wt_json_token(WT_SESSION *wt_session, const char *src, int *toktype,
 ...
 *	__wt_json_token.
 */
const char *__wt_json_tokname(int toktype)
{
	switch (toktype) {
	case 0:		return ("<EOF>");
 ...
 */
static int __json_pack_struct(WT_SESSION_IMPL *session, void *buffer, size_t size,
...
static int __json_pack_size(
    WT_SESSION_IMPL *session, const char *fmt, WT_CONFIG_ITEM *names, int iskey, const char *jstr, size_t *sizep)
{
	WT_CONFIG_ITEM name;
	WT_DECL_PACK_VALUE(pv);
	WT_PACK pack;
 ...
 */
int __wt_json_to_item(WT_SESSION_IMPL *session, const char *jstr,
    const char *format, WT_CURSOR_JSON *json, int iskey, WT_ITEM *item)
{
	size_t sz;
	sz = 0; /* Initialize because GCC 4.1 is paranoid */

	WT_RET(__json_pack_size(session, format, iskey ? &json->key_names : &json->value_names, iskey, jstr, &sz));
 ...
 */
int __wt_json_strncpy(char **pdst, size_t dstlen, const char *src, size_t srclen)
{
	char *dst;
