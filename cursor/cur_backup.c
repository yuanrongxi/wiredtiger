...
static int __backup_all(WT_SESSION_IMPL* session, WT_CURSOR_BACKUP* cursor);
...
static int __backup_start(WT_SESSION_IMPL *, WT_CURSOR_BACKUP *, const char *[]);
static int __backup_stop(WT_SESSION_IMPL*);
static int __backup_uri(WT_SESSION_IMPL *, WT_CURSOR_BACKUP *, const char *[], int *, int *);

/*backup cursor的next方法实现*/
static int __curbackup_next(WT_CURSOR* cursor)
{
	...
	cb->iface.key.size = strlen(cb->list[cb->next].name) + 1; /*要算上最后一个'\0'*/
	++cb->next;
...
static int __curbackup_close(WT_CURSOR* cursor)
{
	...
	WT_WITH_SCHEMA_LOCK(session, tret = __backup_stop(session));		/* Stop the backup. */
	WT_TRET(tret);
...
/*创建并打开一个backup cursor*/
int __wt_curbackup_open(WT_SESSION_IMPL *session, const char *uri, const char *cfg[], WT_CURSOR **cursorp)
{
	...
	*cursor = iface;
	...
	/*启动backup,这个过程是要在获取schema lock的条件下执行的，目的是要在建立backup时保持schema的一直性*/
...
/*将现存所有的日志文件追加到backup list当中*/
static int __backup_log_append(WT_SESSION_IMPL* session, WT_CURSOR_BACKUP* cb, int active)
{
	...
	if(conn->log){
		/*获得所有日志文件的文件名和最大的文件序号id,并将这些文件加入到backup list当中*/
		WT_ERR(__wt_log_get_all_files(session, &logfiles, &logcount, &cb->maxid, active));
...
/*启动一个基于redo log的backup*/
static int __backup_start(WT_SESSION_IMPL* session, WT_CURSOR_BACKUP* cb, const char* cfg[])
{
	WT_CONNECTION_IMPL *conn;
	WT_DECL_RET;
 ...
 *	schema lock held.
 */
static int __backup_cleanup_handles(WT_SESSION_IMPL *session, WT_CURSOR_BACKUP *cb)
{
	...
	for(p = cb->list; p->name != NULL; ++p){
		if (p->handle != NULL)
			WT_WITH_DHANDLE(session, p->handle, WT_TRET(__wt_session_release_btree(session)));
...
/*停止一个基于redo log的backup*/
static int __backup_stop(WT_SESSION_IMPL* session)
{
	WT_CONNECTION_IMPL *conn;
	WT_DECL_RET;
...
static int __backup_all(WT_SESSION_IMPL* session, WT_CURSOR_BACKUP* cb)
{
	WT_CONFIG_ITEM cval;
	WT_CURSOR *cursor;
	WT_DECL_RET;
	...
	/*打开一个session对应的meta cursor，并将meta中的所有信息拷贝到backup file中*/
	...
	while((ret = cursor->next(cursor)) == 0){
		...
		if ((ret = __wt_config_getones(session, value, "type", &cval)) == 0 &&
			!WT_PREFIX_MATCH_LEN(cval.str, cval.len, "file") && !WT_PREFIX_MATCH_LEN(cval.str, cval.len, "lsm"))
			WT_ERR_MSG(session, ENOTSUP, "hot backup is not supported for objects of type %.*s", (int)cval.len, cval.str);
		WT_ERR_NOTFOUND_OK(ret);

		if ((ret =__wt_config_getones(session, value, "source", &cval)) == 0 
			&& !WT_PREFIX_MATCH_LEN(cval.str, cval.len, "file:") && !WT_PREFIX_MATCH_LEN(cval.str, cval.len, "lsm:"))
			WT_ERR_MSG(session, ENOTSUP, "hot backup is not supported for objects of source %.*s", (int)cval.len, cval.str);
		WT_ERR_NOTFOUND_OK(ret);
	}

	WT_ERR_NOTFOUND_OK(ret);

	/*构建需要拷贝的btree file list*/
	WT_WITH_DHANDLE_LOCK(session, ret = __wt_meta_btree_apply(session, __backup_list_all_append, NULL));
...
static int __backup_uri(WT_SESSION_IMPL *session, WT_CURSOR_BACKUP *cb, const char *cfg[], int *foundp, int *log_only)
{
	...
	*log_only = 0;
	...
	for(cb->list_next = 0, target_list = 0; (ret = __wt_config_next(&targetconf, &k, &v)) == 0; ++target_list){
		...
		if (WT_PREFIX_MATCH(uri, "log:")) {
			...
			WT_ERR(__wt_schema_worker(session, uri, NULL, __wt_backup_list_uri_append, cfg, 0));
	}
	WT_ERR_NOTFOUND_OK(ret);
 ...
 */
static int __backup_file_create(WT_SESSION_IMPL *session, WT_CURSOR_BACKUP *cb, int incremental)
{
	return (__wt_fopen(session, incremental ? WT_INCREMENTAL_BACKUP : WT_METADATA_BACKUP,
 ...
 */
int __wt_backup_list_uri_append(WT_SESSION_IMPL *session, const char *name, int *skip)
{
	WT_CURSOR_BACKUP *cb;
	char *value;
 ...
 */
static int __backup_list_all_append(WT_SESSION_IMPL *session, const char *cfg[])
{
	WT_CURSOR_BACKUP *cb;
...
static int __backup_list_append(WT_SESSION_IMPL *session, WT_CURSOR_BACKUP *cb, const char *uri)
{
	...
	if (need_handle) {
		...
		if ((ret = __wt_session_get_btree(session, uri, NULL, NULL, 0)) == 0)
