...
/*获取index cursor对应的value，主要是index对应column的信息*/
static int __cur_get_value(WT_CURSOR* cursor, ...)
{
	...
	if(F_ISSET(cursor, WT_CURSOR_RAW_OK)){
		ret = __wt_schema_project_merge(session, cindex->cg_cursors, cindex->value_plan, cursor->value_format, &cursor->value);
		if (ret == 0) {
		...
		ret = __wt_schema_project_out(session, cindex->cg_cursors, cindex->value_plan, ap);

	va_end(ap);
}

/*设置cursor的值,直接返回了一个ENOTSUP,在index cursor中不能动态改变索引的列组合元信息*/
...
static int __curindex_compare(WT_CURSOR *a, WT_CURSOR *b, int *cmpp)
{
	...
	if (!WT_PREFIX_MATCH(a->uri, "index:") || !WT_PREFIX_MATCH(b->uri, "index:"))
...
static int __curindex_move(WT_CURSOR_INDEX *cindex)
{
	...
	for (i = 0, cp = cindex->cg_cursors; i < WT_COLGROUPS(cindex->table); i++, cp++) {
		if (*cp == NULL)
			continue;
		if (first == NULL) { /*在第一次赋值之前，现将格式化的key做一个展开，然后赋值给这个索引所所有的索引cursor对象*/
			/*
			 ...
			 */
			WT_RET(__wt_schema_project_slice(session, cp, cindex->index->key_plan,
			    1, cindex->index->key_format, &cindex->iface.key));
			first = *cp;
		} else {
...
/*index cursor对象的复位函数,会将当前索引对应的所有cursor对象进行reset*/
...
static int __curindex_search(WT_CURSOR* cursor)
{
	 ...
	 * matches.
	 *先进行近似查找，让后根据KEY值做精确匹配，防止primary key在这个过程被追加后变得不兼容
	 */
...
static int __curindex_search_near(WT_CURSOR *cursor, int *exact)
{
	...
	if ((ret = cindex->child->search_near(cindex->child, exact)) == 0) /*将这个索引对象关联的所有cursor定位到新的位置*/
		ret = __curindex_move(cindex);
	else
		F_CLR(cursor, WT_CURSTD_KEY_INT | WT_CURSTD_VALUE_INT);
...
static int __curindex_close(WT_CURSOR* cursor)
{
	...
	/*对index资源的释放，主要是key_plan/value_plan, k/v format等*/
...
/*打开与对应索引相关的col groups中所有column对应的cursor*/
static int __curindex_open_colgroups(WT_SESSION_IMPL *session, WT_CURSOR_INDEX *cindex, const char *cfg_arg[])
{
	WT_TABLE *table;
	WT_CURSOR **cp;
	u_long arg;
	...
	for (proj = (char *)cindex->value_plan; *proj != '\0'; proj++) {
		...
		if ((*proj != WT_PROJ_KEY && *proj != WT_PROJ_VALUE) ||
			cp[arg] != NULL)
			continue;
		WT_RET(__wt_open_cursor(session, table->cgroups[arg]->source, &cindex->iface, cfg, &cp[arg]));
	}
 ...
 */
int __wt_curindex_open(WT_SESSION_IMPL *session, const char *uri, WT_CURSOR *owner, const char *cfg[], WT_CURSOR **cursorp)
{
	...
	/*将对应table实例化到内存中并增加引用计数*/
	if ((ret = __wt_schema_get_table(session, tablename, namesize, 0, &table)) != 0) {
		if (ret == WT_NOTFOUND)
	...
	/*在内存中构建对应索引的对象，并设置相关参数，例如k/v plan, k/v format等*/
	...
	*cursor = iface;
	...
	if (WT_CURSOR_RECNO(cursor))
		WT_ERR_MSG(session, WT_ERROR, "Column store indexes based on a record number primary key are not supported.");

	/* Handle projections. */
	if (columns != NULL) {
		WT_ERR(__wt_scr_alloc(session, 0, &tmp));
		WT_ERR(__wt_struct_reformat(session, table, columns, strlen(columns), NULL, 0, tmp));
	...
	/*获得索引对应的所有column,并打开这些列对应的cursor对象*/
	WT_ERR(__curindex_open_colgroups(session, cindex, cfg));

	if (F_ISSET(cursor, WT_CURSTD_DUMP_JSON))
		WT_ERR(__wt_json_column_init(cursor, table->key_format, &idx->colconf, &table->colconf));
