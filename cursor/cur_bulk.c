...
static int __curbulk_insert_var(WT_CURSOR* cursor)
{
	...
	*/
	duplicate = 0;
	if (cbulk->rle != 0) {
		if (cbulk->last.size == cursor->value.size && memcmp(cbulk->last.data, cursor->value.data, cursor->value.size) == 0) {
			++cbulk->rle;
			duplicate = 1;
		}
		else
	...
	/*
	* Save a copy of the value for the next comparison and reset the RLE counter.
	*/
	if (!duplicate) {
		WT_ERR(__wt_buf_set(session, &cbulk->last, cursor->value.data, cursor->value.size));
		cbulk->rle = 1;
	}
...
/*抛出一个错误信息，信息携带了cur key/last_data的十六进制值文本内容*/
static int __bulk_row_keycmp_err(WT_CURSOR_BULK* cbulk)
{
	...
	WT_ERR(__wt_buf_set_printable(session, a, cursor->key.data, cursor->key.size));
	WT_ERR(__wt_buf_set_printable(session, b, cbulk->last.data, cbulk->last.size));

	WT_ERR_MSG(session, EINVAL, "bulk-load presented with out-of-order keys: %.*s compares smaller than previously inserted key %.*s",
		(int)a->size, (const char *)a->data, (int)b->size, (const char *)b->data);
err:
...
/*关闭bulk cursor*/
static int __curbulk_close(WT_CURSOR* cursor)
{
	WT_BTREE *btree;
	WT_CURSOR_BULK *cbulk;
	WT_DECL_RET;
...
*/
int __wt_curbulk_init(WT_SESSION_IMPL* session, WT_CURSOR_BULK* cbulk, int bitmap, int skip_sort_check)
{
	WT_CURSOR *c;
	WT_CURSOR_BTREE *cbt;
