...
/*将meta cursor对应的file cursor中的key值拷贝到meta cursor对应的key中*/
...
/*将meta cursor对应的file cursor中的value值拷贝到meta cursor对应的value中*/
...
/*将cursor中的k/v设置到file cursor对应的k/v中*/
...
static int __curmetadata_prev(WT_CURSOR *cursor)
{
	...
	else if (ret == WT_NOTFOUND) /*重新在meta table中定位cursor key的meta对应的位置*/
		WT_ERR(__curmetadata_metadata_search(session, cursor));
...
static int __curmetadata_reset(WT_CURSOR* cursor)
{
	...
	mdc = (WT_CURSOR_METADATA *)cursor;
	file_cursor = mdc->file_cursor;
	CURSOR_API_CALL(cursor, session, reset, ((WT_CURSOR_BTREE *)file_cursor)->btree);
...
static int __curmetadata_close(WT_CURSOR *cursor)
{
	...
	mdc = (WT_CURSOR_METADATA *)cursor;
	file_cursor = mdc->file_cursor;
	CURSOR_API_CALL(cursor, session, close, ((WT_CURSOR_BTREE *)file_cursor)->btree);
...
/*创建并打开一个meta cursor对象*/
int __wt_curmetadata_open(WT_SESSION_IMPL* session, const char* uri, WT_CURSOR* owner, const char* cfg[], WT_CURSOR** cursorp)
{
