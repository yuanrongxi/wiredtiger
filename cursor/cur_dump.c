...
/*将from中的二进制数据转成ASCI文本格式存入to中*/
static int __raw_to_dump(WT_SESSION_IMPL *session, WT_ITEM *from, WT_ITEM *to, int hexonly)
{
	if (hexonly)
...
/*将src_arg中asci描述的数据转成二进制数据存入item中*/
static int __dump_to_raw(WT_SESSION_IMPL* session, const char* src_arg, WT_ITEM* item, int hexonly)
{
	if (hexonly)
...
static int __curdump_get_key(WT_CURSOR* cursor, ...)
{
	...
	if (F_ISSET(cursor, WT_CURSTD_DUMP_JSON)){
		...
		ret = __wt_json_alloc_unpack(session, buffer, size, fmt, json, 1, ap);
	}
	else{
		if (WT_CURSOR_RECNO(cursor) && !F_ISSET(cursor, WT_CURSTD_RAW)) {
			...
			WT_ERR(__raw_to_dump(session, &item, &cursor->key, F_ISSET(cursor, WT_CURSTD_DUMP_HEX) ? 1 : 0));
		}
...
static void __curdump_set_key(WT_CURSOR *cursor, ...)
{
	...
	/*获得不定格式化的字符串，这里面是设置的key值的字符串表达形式*/
	va_start(ap, cursor);
	if (F_ISSET(cursor, WT_CURSTD_RAW))
	...
	if (WT_CURSOR_RECNO(cursor) && !F_ISSET(cursor, WT_CURSTD_RAW)) {
		...
		if (F_ISSET(cursor, WT_CURSTD_DUMP_JSON))
			WT_ERR(__wt_json_to_item(session, p, cursor->key_format, (WT_CURSOR_JSON *)cursor->json_private, 1, &cursor->key));
		else
			WT_ERR(__dump_to_raw(session, p, &cursor->key, F_ISSET(cursor, WT_CURSTD_DUMP_HEX) ? 1 : 0));
...
static int __curdump_get_value(WT_CURSOR *cursor, ...)
{
	...
	if (F_ISSET(cursor, WT_CURSTD_DUMP_JSON)) {
		...
		ret = __wt_json_alloc_unpack(session, item.data, item.size, fmt, json, 0, ap);
	}
	else {
		WT_ERR(child->get_value(child, &item));

		WT_ERR(__raw_to_dump(session, &item, &cursor->value, F_ISSET(cursor, WT_CURSTD_DUMP_HEX) ? 1 : 0));
...
static void __curdump_set_value(WT_CURSOR *cursor, ...)
{
	...
	if (F_ISSET(cursor, WT_CURSTD_DUMP_JSON))
		WT_ERR(__wt_json_to_item(session, p, cursor->value_format, (WT_CURSOR_JSON *)cursor->json_private, 0, &cursor->value));
	else
		WT_ERR(__dump_to_raw(session, p, &cursor->value, F_ISSET(cursor, WT_CURSTD_DUMP_HEX) ? 1 : 0));
...
int __wt_curdump_create(WT_CURSOR *child, WT_CURSOR *owner, WT_CURSOR **cursorp)
{
	...
	*cursor = iface;
	...
	F_SET(cursor, F_ISSET(child, WT_CURSTD_DUMP_HEX | WT_CURSTD_DUMP_JSON | WT_CURSTD_DUMP_PRINT));
	if (F_ISSET(cursor, WT_CURSTD_DUMP_JSON)) {
