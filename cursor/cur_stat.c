 ...
 */
static void __curstat_free_config(WT_SESSION_IMPL *session, WT_CURSOR_STAT *cst)
{
	size_t i;
...
static int __curstat_get_value(WT_CURSOR *cursor, ...)
{
	...
	if (F_ISSET(cursor, WT_CURSTD_RAW)) {
		...
		WT_ERR(__wt_struct_pack(session, cursor->value.mem, size,
		    cursor->value_format, cst->stats_first[WT_STAT_KEY_OFFSET(cst)].desc, cst->pv.data, cst->v));
 ...
 */
int __wt_curstat_init(WT_SESSION_IMPL *session, const char *uri, const char *cfg[], WT_CURSOR_STAT *cst)
{
	const char *dsrc_uri;
	...
	if (WT_PREFIX_MATCH(dsrc_uri, "colgroup:"))
		return (__wt_curstat_colgroup_init(session, dsrc_uri, cfg, cst));
 ...
 */
int __wt_curstat_open(WT_SESSION_IMPL *session, const char *uri, const char *cfg[], WT_CURSOR **cursorp)
{
	WT_CONNECTION_IMPL *conn;
	...
	*cursor = iface;
	...
	if ((ret = __wt_config_gets(session, cfg, "statistics", &cval)) == 0) {
		...
		if ((ret = __wt_config_subgets(session, &cval, "fast", &sval)) == 0 && sval.val != 0) {
			if (F_ISSET(cst, WT_CONN_STAT_ALL))
				WT_ERR_MSG(session, EINVAL, "only one statistics configuration value may be specified");
	...
	if (0) {
config_err:	
		WT_ERR_MSG(session, EINVAL, "cursor's statistics configuration doesn't match the database statistics configuration");
	}
