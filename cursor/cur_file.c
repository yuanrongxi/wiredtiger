...
/*如果执行f函数成功，如果执行失败，cursor回到执行的k /v值*/
...
/*btree cursor的随机定位到btree上的一个记录上*/
static int __curfile_next_random(WT_CURSOR* cursor)
{
	WT_CURSOR_BTREE *cbt;
	WT_DECL_RET;
	WT_SESSION_IMPL *session;
...
static int __curfile_search_near(WT_CURSOR *cursor, int *exact)
{
	...
	WT_BTREE_CURSOR_SAVE_AND_RESTORE(cursor, __wt_btcur_search_near(cbt, exact), ret);
...
int __wt_curfile_update_check(WT_CURSOR *cursor)
{
	WT_CURSOR_BTREE *cbt;
	WT_DECL_RET;
	WT_SESSION_IMPL *session;
	...
	WT_BTREE_CURSOR_SAVE_AND_RESTORE(cursor, __wt_btcur_update_check(cbt), ret);
...
static int __curfile_remove(WT_CURSOR* cursor)
{
	 ...
	 */
	if(ret == 0){
		if (F_ISSET(cursor, WT_CURSTD_KEY_INT) && !WT_DATA_IN_ITEM(&(cursor)->key)) {
				WT_ERR(__wt_buf_set(session, &cursor->key, cursor->key.data, cursor->key.size));
				F_CLR(cursor, WT_CURSTD_KEY_INT);
				F_SET(cursor, WT_CURSTD_KEY_EXT);
		}
...
/*创建一个btree cursor*/
int __wt_curfile_create(WT_SESSION_IMPL *session,
...
int __wt_curfile_open(WT_SESSION_IMPL *session, const char *uri, WT_CURSOR *owner, const char *cfg[], WT_CURSOR **cursorp)
{
	...
	if (cval.type == WT_CONFIG_ITEM_BOOL ||(cval.type == WT_CONFIG_ITEM_NUM && (cval.val == 0 || cval.val == 1))) {
		bitmap = 0;
		bulk = (cval.val != 0);
	} 
