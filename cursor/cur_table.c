...
/*让table相关的colgroups cursor对象执行一个f操作*/
...
/*产生一个extractor key，并将这个key设置到extractor cursor中，进行对应的extractor操作*/
static int __curextract_insert(WT_CURSOR* cursor)
{
	...
	session = (WT_SESSION_IMPL *)cursor;
	...
	/*将index cursor的key赋值为extractor key和colgroup cursor key的拼接*/
	...
	/*通过新的index cursor key执行cextractor函数来更新索引对象*/
	return (cextract->f(cextract->idxc));
}

/*让表对象中所有的index cursor执行一个操作,这个操作对应的函数通过func_off来定位*/
static int __apply_idx(WT_CURSOR_TABLE* ctable, size_t func_off, int skip_immutable)
{
	...
	for(i = 0; i < ctable->table->nindices; i++, cp++){
		...
		if (idx->extractor) {
			...
			ret = idx->extractor->extract(idx->extractor, &session->iface, &key, &value, &extract_cursor.iface);

			__wt_buf_free(session, &extract_cursor.iface.key);
			WT_RET(ret);
		} 
		else {
			WT_RET(__wt_schema_project_merge(session, ctable->cg_cursors, idx->key_plan, idx->key_format, &(*cp)->key));
			/*
...
int __wt_curtable_get_value(WT_CURSOR *cursor, ...)
{
	...
	if (F_ISSET(cursor, WT_CURSOR_RAW_OK)) {
		ret = __wt_schema_project_merge(session, ctable->cg_cursors, ctable->plan, cursor->value_format, &cursor->value);
		if (ret == 0) {
		...
		ret = __wt_schema_project_out(session, ctable->cg_cursors, ctable->plan, ap);
	va_end(ap);
...
void __wt_curtable_set_value(WT_CURSOR* cursor, ...)
{
	...
	if (F_ISSET(cursor, WT_CURSOR_RAW_OK | WT_CURSTD_DUMP_JSON)) {
		...
		ret = __wt_schema_project_slice(session, ctable->cg_cursors, ctable->plan, 0, cursor->value_format, &cursor->value);
	} else {
		/*
...
/*table cursor根据primary column来进行下一个next定位*/
...
static int __curtable_update(WT_CURSOR* cursor)
{
	...
	if(ctable->table->nindices > 0){
		...
		WT_ERR(__wt_schema_project_merge(session, ctable->cg_cursors, ctable->plan, cursor->value_format, value_copy));
		APPLY_CG(ctable, search);

		/* Remove only if the key exists. 先从索引中删除，再添加*/
		if (ret == 0) {
			WT_ERR(__apply_idx(ctable, offsetof(WT_CURSOR, remove), 1));
			/*重新恢复删除前所有colgroup cursor的value值？*/
			WT_ERR(__wt_schema_project_slice(session, ctable->cg_cursors, ctable->plan, 0, cursor->value_format, value_copy));
		} 
		else
...
int __wt_table_range_truncate(WT_CURSOR_TABLE* start, WT_CURSOR_TABLE* stop)
{
	...
	if(ctable->table->nindices > 0){
		if (start == NULL) { /*将stop前面的数据记录全部删除*/
			...
			do {
				...
				WT_ERR(ret);
				WT_ERR(__apply_idx(stop, offsetof(WT_CURSOR, remove), 0));
			...
			do {
				...
				WT_ERR(ret);
				WT_ERR(__apply_idx(start, offsetof(WT_CURSOR, remove), 0));
				if (stop != NULL)
	...
	for (i = 0; i < WT_COLGROUPS(ctable->table); i++)
		WT_ERR(__wt_range_truncate((start == NULL) ? NULL : start->cg_cursors[i], (stop == NULL) ? NULL : stop->cg_cursors[i]));
...
static int __curtable_open_colgroups(WT_CURSOR_TABLE *ctable, const char *cfg_arg[])
{
	...
	if (!complete)
		WT_RET_MSG(session, EINVAL, "Can't use '%s' until all column groups are created", table->name);
...
static int __curtable_open_indices(WT_CURSOR_TABLE* ctable)
{
	...
	for (i = 0, cp = ctable->idx_cursors; i < table->nindices; i++, cp++)
		WT_RET(__wt_open_cursor(session, table->indices[i]->source, &ctable->iface, ctable->cfg, cp));
	return 0;
}

/*创建并打开table cursor*/
int __wt_curtable_open(WT_SESSION_IMPL *session, const char *uri, const char *cfg[], WT_CURSOR **cursorp)
{
	WT_CURSOR_STATIC_INIT(iface,
	...
	*cursor = iface;
	...
	if(columns != NULL){
		WT_ERR(__wt_struct_reformat(session, table, columns, strlen(columns), NULL, 1, tmp));
		WT_ERR(__wt_strndup(session, tmp->data, tmp->size, &cursor->value_format));

		WT_ERR(__wt_buf_init(session, tmp, 0));
		WT_ERR(__wt_struct_plan(session, table, columns, strlen(columns), 0, tmp));
	...
	/*创建并初始化table cursor，并设置默认的cursor属性*/
	WT_ERR(__wt_cursor_init(cursor, cursor->internal_uri, NULL, cfg, cursorp));
