...
/*分配一个bloom对象，并用config对其初始化*/
static int __bloom_init(WT_SESSION_IMPL* session, const char* uri, const char* config, WT_BLOOM** bloomp)
{
	WT_BLOOM *bloom;
	WT_DECL_RET;
	size_t len;
	...
	*bloomp = NULL;
	...
	snprintf(bloom->config, len, "%s,%s", config == NULL ? "" : config, WT_BLOOM_TABLE_CONFIG);
...
static int __bloom_setup(WT_BLOOM* bloom, uint64_t n, uint64_t m, uint32_t factor, uint32_t k)
{
	...
	if(n != 0){
		bloom->n = n;
		bloom->m = n * bloom->factor;
	}
	else{
		bloom->m = m;
		bloom->n = m / bloom->factor;
	}
...
/*创建一个bloom 过滤器，并初始化它*/
int __wt_bloom_create(WT_SESSION_IMPL *session, const char *uri, const char *config, 
...
err:
	__wt_bloom_close(bloom);
	return ret;
}

/*根据bloom的配置参数，创建一个bloom需要的cursor*/
...
/*打开个bloom过滤器对象，并为其创建一个cursor,可能是从sstable中将导入bloom信息*/
int __wt_bloom_open(WT_SESSION_IMPL *session, const char *uri, uint32_t factor, uint32_t k, WT_CURSOR *owner, WT_BLOOM **bloomp)
{
	WT_BLOOM *bloom;
	WT_CURSOR *c;
	WT_DECL_RET;
...
/*设置一个KEY值的bloom过滤值*/
int __wt_bloom_insert(WT_BLOOM *bloom, WT_ITEM *key)
{
	uint64_t h1, h2;
	uint32_t i;
...
/*对key做bloom hash操作*/
int __wt_bloom_hash(WT_BLOOM *bloom, WT_ITEM *key, WT_BLOOM_HASH *bhash)
{
	WT_UNUSED(bloom);
...
/*判断bhash对应的值是否在bloom过滤器中,如果在其中，返回为0*/
...
/*判断key是否在bloom过滤器中*/
int __wt_bloom_get(WT_BLOOM *bloom, WT_ITEM *key)
{
	WT_BLOOM_HASH bhash;
...
/*关闭bloom过滤器(仅仅是内存中销毁)*/
int __wt_bloom_close(WT_BLOOM *bloom)
{
	WT_DECL_RET;
	WT_SESSION_IMPL *session;
