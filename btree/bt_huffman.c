...
static int __wt_huffman_read(WT_SESSION_IMPL*, WT_CONFIG_ITEM*, struct __wt_huffman_table**, u_int*, u_int*);

/* 检查huffman相关的配置信息,并返回配置文件名，在这个过程会打开配置文件句柄 */
static int __huffman_confchk_file(WT_SESSION_IMPL *session, WT_CONFIG_ITEM *v, int *is_utf8p, FILE **fpp)
{
	FILE *fp;
	WT_DECL_RET;
	size_t len;
	...
	WT_ERR(__wt_fopen(session, fname, WT_FHANDLE_READ, WT_FOPEN_FIXED, &fp));
...
/* huffman 配置文件合法性检查 */
int __wt_huffman_confchk(WT_SESSION_IMPL *session, WT_CONFIG_ITEM *v)
{
	if(v->len == 0)
		return 0;

	/*默认是英语方式编码，不需要进行配置文件的校验*/
...
int __wt_btree_huffman_open(WT_SESSION_IMPL *session)
{
	...
	case BTREE_COL_VAR:
		if (key_conf.len != 0)
			WT_RET_MSG(session, EINVAL, "the keys of variable-length column-store files may not be Huffman encoded");
		break;
	...
	if(key_conf.len == 0)
		...
		WT_RET(__wt_huffman_open(session, copy, WT_ELEMENTS(__wt_huffman_nytenglish), 1, &btree->huffman_key));
		...
		/* Check for a shared key/value table. */
		if (value_conf.len != 0 && key_conf.len == value_conf.len && memcmp(key_conf.str, value_conf.str, key_conf.len) == 0) {
			btree->huffman_value = btree->huffman_key;
			return 0;
		}
	}
	...
	if (value_conf.len == 0) {
		...
		memcpy(copy, __wt_huffman_nytenglish, sizeof(__wt_huffman_nytenglish));
		WT_RET(__wt_huffman_open(session, copy, WT_ELEMENTS(__wt_huffman_nytenglish), 1, &btree->huffman_value));
	} 
	else {
...
static int __wt_huffman_read(WT_SESSION_IMPL *session, WT_CONFIG_ITEM *ip,
							struct __wt_huffman_table **tablep, u_int *entriesp, u_int *numbytesp)
{
	...
	*entriesp = *numbytesp = 0;
	...
	for (tp = table, lineno = 1; (ret = fscanf(fp, "%" SCNi64 " %" SCNi64, &symbol, &frequency)) != EOF; ++tp, ++lineno) {
		if (lineno > entries)
			WT_ERR_MSG(session, EINVAL, "Huffman table file %.*s is corrupted, more than %" PRIu32 " entries", (int)ip->len, ip->str, entries);
		if (ret != 2)
			WT_ERR_MSG(session, EINVAL,"line %u of Huffman table file %.*s is corrupted: expected two unsigned integral values",
			lineno, (int)ip->len, ip->str);
		if (symbol < 0 || symbol > entries)
			WT_ERR_MSG(session, EINVAL,
			"line %u of Huffman file %.*s is corrupted; symbol %" PRId64 " not in range, maximum "
			"value is %u",
...
err:
	...
	__wt_fclose(&fp, WT_FHANDLE_READ);
	return ret;

}

/* 关闭一个huffman编码对象 */
int __wt_btree_huffman_close(WT_SESSION_IMPL* session)
{
	WT_BTREE *btree;
	btree = S2BT(session);
